...
def check_element(periodic_table_dict, symbol, expected):
    ...
    assert act_name == exp_name, (
        f'wrong name for "{symbol}": ' f"expected {exp_name} but found {act_name}"
    )
...
def test_compute_molar_mass():
    ...
    assert compute_molar_mass([["C", 6], ["H", 6]], periodic_table_dict) == approx(
        78.11184
    )
    assert compute_molar_mass(
        [["C", 13], ["H", 16], ["N", 2], ["O", 2]], periodic_table_dict
    ) == approx(232.27834)
