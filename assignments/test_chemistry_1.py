...
def check_element(actual, expected):
    ...
    assert actual[NAME_INDEX] == name, f"{name} is missing from the periodic table."
    ...
    assert act_symbol == exp_symbol, (
        f"wrong symbol for {name}: " f"expected {exp_symbol} but found {act_symbol}."
    )

    # Verify that the element's atomic mass is correct.
    act_mass = actual[ATOMIC_MASS_INDEX]
    exp_mass = expected[ATOMIC_MASS_INDEX]
    assert act_mass == approx(exp_mass, abs=0.01), (
        f"wrong atomic mass for {name}: " f"expected {exp_mass} but found {act_mass}"
    )
