# I defined the constants at the top of the file.

# I added and tested the psi_from_kpi function and displayed the water pressure in kilopascals (kPa) and pounds per square inch (psi).
...
PRESSURE_CONVERSION_FACTOR = 0.14503773773020923
...
def pressure_loss_from_pipe(
    ...
    p = (-friction_factor * pipe_length * WATER_DENSITY * fluid_velocity**2) / (
        2000 * pipe_diameter
    )
    return p
...
def psi_from_kpa(kpa):
    return kpa * PRESSURE_CONVERSION_FACTOR
...
def main():
    ...
    pressure += loss
    psi = psi_from_kpa(pressure)
    print(
        f"Pressure at house: {pressure:.1f} kilopascals equivalent to {psi:.1f} pounds per square inch"
    )
